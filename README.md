...
In code, you can import `parse_components` from [tools/parse_components.py](tools/parse_components.py) and load/merge files however your runtime prefers. When using the Python package, load border characters from the design assets with `askee_ds.box_drawing.load_box_drawing(root=None)` so implementations use the approved set (see [design/ascii/box-drawing.yaml](design/ascii/box-drawing.yaml)) instead of hardcoding.
...
AskeeDS uses or recommends the following for development and optional features:

| Name | Use | Link | License |
|------|-----|------|---------|
| **PyYAML** | Parsing YAML (box-drawing, maps, manifest). | [PyYAML](https://pyyaml.org/) | MIT |
| **Textual** | Optional: interactive component visual test (`pip install -e ".[visual-test]"`; includes pyfiglet for Figlet font browser). | [Textual](https://textual.textualize.io/) | MIT |
| **pyfiglet** | Optional: Figlet-style banner text for the **typography.banner** component (`pip install -e ".[banner]"`). Only banner/special content uses Figlet; all other TUI text is plain. | [pyfiglet](https://github.com/pwaller/pyfiglet) | GPL-2.0 |
| **FIGlet** | Original C implementation; pyfiglet is a Python port. | [FIGlet](http://www.figlet.org/) | BSD-style |
